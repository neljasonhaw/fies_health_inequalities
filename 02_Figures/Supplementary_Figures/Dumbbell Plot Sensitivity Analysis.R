###############################################################################
###    DUMBBELL PLOTS OF FIES 2015 ORIGINAL VS REVISED POVERTY THRESHOLD    ###
...
data_dumbbell <- read_dta("dumbbell_sensi.dta")
## Create a new column that combines the pov_thres round
## Select columns
data_dumbbell <- data_dumbbell %>% select(outcome, pov_thres, nonpoor, poor, diff, N)
...
##         select(pov_thres, nonpoor, poor, diff)})
###############################################################################
...
dumbbell_pov <- function(i) {
  ...
  i <- ggplot() +
    ...
    geom_dumbbell(data = i,
                  aes(y = pov_thres, x = nonpoor, xend = poor),
                  size = 2, color = "gray50", 
...
all_dumbbell$prev_tobacco <- all_dumbbell$prev_tobacco +
  geom_text(data = filter(data_dumbbell, pov_thres == "Revised" & outcome == "prev_tobacco"),
            aes(x = nonpoor, y = "Revised", label = "NONPOOR"),
            color = nonpoor_navy, size = 5, vjust = -1.5, hjust = 0.75,
            fontface = "bold", family = "HelveticaNeueforSAS") +
  geom_text(data = filter(data_dumbbell, pov_thres == "Revised" & outcome == "prev_tobacco"),
            aes(x = poor, y = "Revised", label = "POOR"),
  ...
  geom_text(data = filter(data_dumbbell, outcome == "prev_tobacco"),
            aes(x = nonpoor, y = pov_thres, label = format(nonpoor*100, digits = 3)),
            color = nonpoor_navy, size = 5, vjust = 2,
            family = "HelveticaNeueforSAS") +
  geom_text(data = filter(data_dumbbell, outcome == "prev_tobacco"),
            aes(x = poor, y = pov_thres, label = format(poor*100, digits = 3)),
...
all_dumbbell$share_tobacco_totex <- all_dumbbell$share_tobacco_totex +
  ...
  geom_text(data = filter(data_dumbbell, outcome == "share_tobacco_totex"),
            aes(x = nonpoor, y = pov_thres, label = format(nonpoor*100, digits = 2),
                hjust = ifelse(abs(diff) < 0.001, 1, 0.5)),
            color = nonpoor_navy, size = 5, vjust = 2,
            family = "HelveticaNeueforSAS") +
  geom_text(data = filter(data_dumbbell, outcome == "share_tobacco_totex"),
            aes(x = poor, y = pov_thres, label = format(poor*100, digits = 2),
...
all_dumbbell$tobacco_2018 <- all_dumbbell$tobacco_2018 +
  ...
  geom_text(data = filter(data_dumbbell, outcome == "tobacco_2018"),
            aes(x = nonpoor, y = pov_thres, label = format(nonpoor, digits = 4, big.mark = ","),
                hjust = ifelse(abs(diff) < 2000, 0, 0.5)),
            color = nonpoor_navy, size = 5, vjust = 2,
            family = "HelveticaNeueforSAS") +
  geom_text(data = filter(data_dumbbell, outcome == "tobacco_2018"),
            aes(x = poor, y = pov_thres, label = format(poor, digits = 4, big.mark = ","),
...
ggsave("Supplementary_Figures/dumbbell_plot_sensi.tiff", plot = dumbbell_plot, width = 24, height = 15, units = "cm", scale = 1.75)
###############################################################################
